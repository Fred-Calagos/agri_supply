...
class Order extends Model
{
    ...
    public static function getAllOrders() {
            ...
            GROUP_CONCAT(DISTINCT pb.selling_price SEPARATOR ', ') AS selling_prices,
            GROUP_CONCAT(DISTINCT pb.stocks SEPARATOR ', ') AS stocks,
        ...
        FROM " . self::$table . " po 
        JOIN product_batch pb ON po.batch_id = pb.id
    ...
    public static function create($data) {
        ...
        $sql = "INSERT INTO " . self::$table . " (user_id, batch_id, product_quantity, order_status, order_track, payment_id) 
                VALUES (:user_id, :batch_id, :product_quantity, :order_status, :order_track, :payment_id)";
    ...
    public static function soldProducts($id)
    {
        ...
        $stmt = $pdo->prepare("
            ...
            FROM " . self::$table . " po
            JOIN product_batch pb ON po.batch_id = pb.id
            JOIN products p ON pb.product_id = p.id -- Fixed the JOIN condition
            WHERE po.order_status = 4 AND po.batch_id = :id
            GROUP BY po.batch_id, p.product_name
            ORDER BY total_sold DESC
        ");
        $stmt->execute(['id' => $id]);
        $result = $stmt->fetch(PDO::FETCH_ASSOC);
        return $result['total_sold'] ?? 0; // Ensure it always returns an integer
    }
    ...
    public static function totalSalesByMonth()
    {
        ...
        $stmt = $pdo->prepare("
            SELECT 
                ...
                pc.product_category,
                SUM(pb.selling_price * po.product_quantity) AS total_sales
            FROM " . self::$table . " po
            JOIN product_batch pb ON po.batch_id = pb.id
            JOIN products p ON pb.product_id = p.id
            JOIN product_category pc ON p.product_category_id = pc.id
    ...
    public static function totalSalesThisMonth()
    {
        $pdo = Database::connect();
        $query = "
            SELECT COALESCE(SUM(pb.selling_price * po.product_quantity), 0) AS total_sales
            FROM " . self::$table . " po
            JOIN product_batch pb ON po.batch_id = pb.id
            WHERE po.order_status = 4
...
public static function totalSalesToday()
{
    $pdo = Database::connect();
    $query = "
        SELECT SUM(pb.selling_price * po.product_quantity) AS total_sales
        FROM " . self::$table . " po
        JOIN product_batch pb ON po.batch_id = pb.id
        WHERE po.order_status = 4 
